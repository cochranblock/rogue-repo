...
- **Coming Soon cards styled** — `.app-card--coming-soon` class: opacity 0.5, pointer-events none. Non-playable games dimmed and non-interactive. Each links to its blocking dependency ([Pixel Forge](https://github.com/cochranblock/pixel-forge)).
- **Login/Logout nav toggle** — f90 takes `Option<Uuid>`. Server-side conditional rendering — Login shown when not authenticated, Logout when authenticated. Fixed the "both visible" issue from d1 Simulation 1.
- **Pixel Forge dependency notice** — Gold-bordered notice above game grid: "Game development begins after Pixel Forge ships. All game assets will be AI-generated by our own model."
- **Economy Coming Soon** — Links to [Stripe↔ISO 8583 bridge](../rogue-repo/src/switch/stripe.rs) as the blocking dependency.
- **WebP icons** — 57MB PNGs → 85KB WebPs (192x192 at q80). 670x reduction. Page load dramatically faster.
- **Session enforcement** — f125 (OnceLock startup check). f126 (require_session on mutation routes). Auth stack now complete through to endpoint protection.
